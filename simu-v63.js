...
function updateScoreDisplay() {
  requestAnimationFrame(() => {
    ...
    updateHiddenFields(frenchTotal, englishTotal)
  })
}

// Update hidden form fields for data capture
function updateHiddenFields(frenchTotal, englishTotal) {
...
function downloadResults() {
  try {
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Résultats Simulateur Arrima - ${resultsData.timestamp}</title>
        <style>
          ...
          .disclaimer { font-size: 12px; color: #6b7280; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; }
        </style>
      </head>
      <body>
        <div class="header">
          ...
          <p>Résultats générés le ${resultsData.timestamp}</p>
        </div>
        
        <div class="total-score">SCORE TOTAL: ${resultsData.totalScore} points</div>
        
        <div class="score-section">
          <div class="category-title">Compétences linguistiques</div>
          <div class="score-item"><span>Compréhension orale français:</span><span>${scores.frenchOralComprehension} pts</span></div>
          <div class="score-item"><span>Production orale français:</span><span>${scores.frenchOralProduction} pts</span></div>
          <div class="score-item"><span>Compréhension écrite français:</span><span>${scores.frenchWrittenComprehension} pts</span></div>
          <div class="score-item"><span>Production écrite français:</span><span>${scores.frenchWrittenProduction} pts</span></div>
          <div class="score-item"><strong>Sous-total français:</strong><strong>${resultsData.frenchTotal} pts</strong></div>
          <div class="score-item"><span>Anglais (toutes compétences):</span><span>${resultsData.englishTotal} pts</span></div>
        </div>
        
        <div class="score-section">
          <div class="category-title">Capital humain</div>
          <div class="score-item"><span>Âge:</span><span>${scores.age} pts</span></div>
          <div class="score-item"><span>Niveau de scolarité:</span><span>${scores.education} pts</span></div>
          <div class="score-item"><span>Domaine de formation:</span><span>${scores.fieldOfStudy} pts</span></div>
          <div class="score-item"><span>Expérience de travail:</span><span>${scores.workExperience} pts</span></div>
        </div>
        
        <div class="score-section">
          <div class="category-title">Facteurs Québec</div>
          <div class="score-item"><span>Diplôme du Québec:</span><span>${scores.quebecDiploma} pts</span></div>
          <div class="score-item"><span>Expérience de travail au Québec:</span><span>${scores.quebecWorkExperience} pts</span></div>
        </div>
        ...
        ${
          hasSpouse
            ? `
        <div class="score-section">
          <div class="category-title">Facteurs conjoint</div>
          <div class="score-item"><span>Compétences orales françaises du conjoint:</span><span>${scores.spouseFrenchOral} pts</span></div>
          <div class="score-item"><span>Âge du conjoint:</span><span>${scores.spouseAge} pts</span></div>
          <div class="score-item"><span>Éducation du conjoint:</span><span>${scores.spouseEducation} pts</span></div>
          <div class="score-item"><span>Domaine d'études du conjoint:</span><span>${scores.spouseFieldOfStudy} pts</span></div>
        </div>
        `
            : ""
        }
        ...
        <div class="score-section">
          ...
          <div class="score-item"><span>Offre d'emploi:</span><span>${scores.jobOffer} pts</span></div>
          <div class="score-item"><span>Enfants à charge:</span><span>${scores.children} pts</span></div>
          <div class="score-item"><span>Famille proche au Québec:</span><span>${scores.familyInQuebec} pts</span></div>
        </div>
    ...
    a.href = url
    a.download = `Resultats-Arrima-${resultsData.timestamp}.html`
