...
function updateSpouseFactorsVisibility() {
  const spouseTitle = document.getElementById("spouse-factors-title")
  const spouseItems = document.getElementById("spouse-factors-items")

  if (spouseTitle && spouseItems) {
    if (hasSpouse) {
      spouseTitle.style.display = "block"
      spouseItems.style.display = "block"
    } else {
      spouseTitle.style.display = "none"
      spouseItems.style.display = "none"
    }
  }
}
