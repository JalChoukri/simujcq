...
function updateScoreDisplay() {
  ...
  if (totalScoreElement) {
    ...
    totalScoreElement.className = "total-value"
    if (totalScore < 300) {
      totalScoreElement.classList.add("text-red-600")
    } else if (totalScore < 600) {
      totalScoreElement.classList.add("text-yellow-600")
    } else {
      totalScoreElement.classList.add("text-green-600")
    }
  }
}
...
function updateFinancialStatus() {
  ...
  if (!financialAutonomyAccepted) {
    financialStatus.innerHTML = `
      <div style="border: 1px solid #fecaca; background-color: #fef2f2; border-radius: 0.5rem; padding: 1rem;">
        <p style="color: #b91c1c; font-weight: 500; font-size: 0.75rem;">
    ...
    financialStatus.innerHTML = `
      <div style="border: 1px solid #bbf7d0; background-color: #f0fdf4; border-radius: 0.5rem; padding: 1rem;">
        <p style="color: #15803d; font-size: 0.75rem;">
...
function generateRecommendations() {
  ...
  if (scoreInterpretationElement) {
    scoreInterpretationElement.innerHTML = `<p class="interpretation-text">${scoreInterpretation}</p>`
  }
  ...
  if (recommendationsList) {
    if (recommendations.length > 0) {
      let recommendationsHTML =
        '<h4 class="recommendations-subtitle">Recommandations personnalisées :</h4><div style="display: grid; gap: 1rem;">'

      recommendations.forEach((rec) => {
        const borderColor =
          rec.priority === "high" ? "border-left: 4px solid #ef4444;" : "border-left: 4px solid #eab308;"
        recommendationsHTML += `
          <div style="display: flex; align-items: flex-start; gap: 0.75rem; padding: 1rem; background: white; border: 1px solid #e5e7eb; border-radius: 0.5rem; ${borderColor}">
            <span style="font-size: 1.125rem;">${rec.icon}</span>
            <div>
              <h5 style="font-size: 0.875rem; font-weight: 600; margin-bottom: 0.25rem;">${rec.title}</h5>
              <p style="font-size: 0.75rem; color: #4b5563;">${rec.description}</p>
            </div>
          </div>
        `
      })
      ...
      recommendationsList.innerHTML = `
        <h4 class="recommendations-subtitle">Recommandations personnalisées :</h4>
        <div style="display: grid; gap: 1rem;">
          <div style="display: flex; align-items: flex-start; gap: 0.75rem; padding: 1rem; background: white; border: 1px solid #e5e7eb; border-radius: 0.5rem; border-left: 4px solid #22c55e;">
            <span style="font-size: 1.125rem;">🎉</span>
            <div>
              <h5 style="font-size: 0.875rem; font-weight: 600; margin-bottom: 0.25rem;">Excellent profil !</h5>
              <p style="font-size: 0.75rem; color: #4b5563;">Votre profil est très solide. Continuez à maintenir vos compétences et restez informé des tirages Arrima.</p>
            </div>
          </div>
        </div>
      `
