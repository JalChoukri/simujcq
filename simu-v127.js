...
// WEBHOOK INTEGRATION - Modified function to send data to Make.com
async function handleLeadMagnetSubmit(event) {
  ...
  try {
    // Calculate total score for additional context
    const totalScore = Object.values(scores).reduce((sum, score) => sum + score, 0)
    ...
    const webhookData = {
      ...
      source: "Arrima Calculator Lead Magnet"
    }

    // Send data to Make.com webhook
    const response = await fetch("https://hook.eu2.make.com/cvxlt8bumn11o3shf9ordtjy1f399r15", {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      ...
      console.log("Lead successfully sent to Make.com")
    } else {
